...
module.exports.run = async (bot, message) => {
    ...
    try {
        ...
        var msg = await channel.messages.fetch(message.content.toLowerCase().split(" ")[2]);
        if (!msg)
        ...
        if (msg.embeds[0].footer.text != "Ended")
            return message.channel.send("That giveaway has not ended!");
        var filter = m => (m.content.toLowerCase() == "confirm" || m.content.toLowerCase() == "cancel");
        var cancelled = false;
        if (msg)
            await message.channel.send("This will immediately re-end that giveaway and select new winners\n`Say confirm or cancel`").then(async () => {
        ...
        try {
            ...
            var temp = "";
            for (var i = 0; i < embd.fields[0].value.split(",").length - 1; i++) {
                var temp3;
                do {
                    temp2 = `${userarray[Math.floor(mt.random() * userarray.length)]}, `;
                    temp3 = await msg.guild.members.fetch(temp2.replace(/\D/g, ''));
                } while (((temp.indexOf(temp2) != -1) || (temp3.id == "[testing bot id]" || temp3.id == "[bot id]")) && ((!embd.fields[2] || !embd.fields[2].value) || temp3.roles.cache.has(embd.fields[2].value.replace(/\D/g, ''))));
                temp += temp2;
            }
